/* 码风说明：
请按我的 C++ 竞赛码风生成代码：
1.使用我给出的固定头文件和 `work()` + 多测 `T` 框架，不要省略测试组数 `T`，写成除非我明确说明单测。
...
8.main函数里不要写lambda之类的，如果是预处理，那就设置一个函数接口例如 prims(); 然后把 prims(); 写成全局函数。
9. 代码逻辑优先写在 `work()` 内，辅助逻辑优先使用 lambda 表达式而不是全局函数。
10.对于需要频繁提前返回的特判，习惯在 `work()` 内定义局部 lambda，例如：
